...
if($file_size1 > 5242880 or $file_size2 > 5242880 or $file_size3 > 5242880){
	echo "<script>alert('Saiz file terlalu besar. Maximum adalah 5 MB.');location.href='permohonan.php';</script>";
}else{
	...
	$sql = "INSERT INTO `permohonanjbi` 
			...
			`negeri`, `jenispermohonan`, `jenismajlis`, `namapemohon`, `notelpemohon`, `kategoripemohon`, `jumlahjbi`, `kemahiranbahasa`, `kemahiranvo`, `emelpemohon`, `statuspermohonan`) 
			VALUES 
			...
			'$kemahiranvo', '$chk','$emelpemohon', 'SEDANG DIPROSES');
	...
	if($rs){
		...
		echo "<script> location.href='permohonanberjaya.php'; </script>";
	}
	else {
		echo "<script> location.href='permohonangagal.php'; </script>";
	}
